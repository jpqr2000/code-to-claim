...
const ReservationSuccess = () => {
  ...
  useEffect(() => {
    // Seguridad: No permitir acceso directo sin userId
    if (!userId) {
      console.warn("Acceso no autorizado a pantalla de éxito sin userId");
      navigate("/");
      return;
    }
  ...
  const loadReservationData = async () => {
    try {
      ...
      // Si hay error de seguridad o datos, redirigir al inicio
      console.warn("No se pudo cargar información de reserva, redirigiendo por seguridad");
      navigate("/");
    } finally {
      setLoading(false);
    }
      ...
      <Card className="relative z-10 w-full max-w-md mx-auto glass-primary animate-scale-in border-2 border-dashed border-event-primary/30 p-6">
        ...
        <div className="text-center mb-6">
          ...
          <div className="inline-flex items-center px-3 py-1 bg-event-success/10 border border-event-success/30 rounded-full">
            <Sparkles className="w-3 h-3 text-event-success mr-1" />
            <span className="text-xs font-medium text-event-success">Ticket Digital</span>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="text-center glass-secondary p-4 rounded-lg border border-event-warning/30">
            ...
            <div className="text-xs text-muted-foreground">
              Conserva este código para futuras consultas
            </div>
          </div>
        </div>
          ...
          <div className="text-center">
            <div className="inline-flex items-center px-3 py-1 bg-event-success/5 border border-event-success/20 rounded-full">
              ...
              <span className="text-xs text-event-success font-medium">
                Confirmado {new Date().toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>
          </div>
        </div>
